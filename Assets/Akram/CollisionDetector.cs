...
public class CollisionDetector : MonoBehaviour
{
    ...
    /// Returns true if collision detected, outputs normal and hit point
    public bool CheckSphereCollision(Vector3 spherePos, float sphereRadius, out Vector3 normal, out Vector3 hitPoint)
    {
        normal = Vector3.zero;
        ...
        {
            Vector3 n, p;
            if (CheckSphereAABB(spherePos, sphereRadius, aabb, out n, out p))
            {
                normal = n;
                hitPoint = p;
                hasCollision = true;
    ...
    bool CheckSphereAABB(Vector3 spherePos, float sphereRadius, AABBData aabb, out Vector3 normal, out Vector3 hitPoint)
    {
        ...
        if (distSq < sphereRadius * sphereRadius)
        {
            ...
            else
            {
                ...
                if (Mathf.Approximately(minDist, toMin.x))
                    normal = Vector3.left;
                else if (Mathf.Approximately(minDist, toMax.x))
                    normal = Vector3.right;
                else if (Mathf.Approximately(minDist, toMin.y))
                    normal = Vector3.down;
                else if (Mathf.Approximately(minDist, toMax.y))
                    normal = Vector3.up;
                else if (Mathf.Approximately(minDist, toMin.z))
                    normal = Vector3.back;
                else
