...
/// Lightweight physics body with rotation support (no Unity Rigidbody)
/// Implements linear and angular dynamics for realistic fragment motion
public class CustomRB
{
    public Vector3 Position { get; private set; }
    public Vector3 Velocity { get; private set; }
    public Quaternion Rotation { get; private set; }
    public Vector3 AngularVelocity { get; private set; } // radians per second
    public float Mass { get; private set; }

    private float gravity = 9.81f;
    private Vector3 inertiaTensor; // Simplified diagonal inertia tensor
    ...
    public CustomRB(Vector3 startPos, float mass, Quaternion startRot, Vector3 dimensions)
    {
        ...
        // Calculate inertia tensor for a box (I = m/12 * (h² + d²), etc.)
        float m = Mass;
    ...
    {
        // Convert torque to angular acceleration using inertia tensor
        // τ = I * α, so α = τ / I
    ...
    {
        // Damping factor: v_new = v_old * (1 - damping * dt)
    ...
    public void Integrate(float dt)
    {
        ...
        // Angular integration - make sure rotation is actually being applied
        if (AngularVelocity.sqrMagnitude > 1e-6f)
        {
            float angle = AngularVelocity.magnitude * dt;
            Vector3 axis = AngularVelocity.normalized;

            // Convert angle from radians to degrees for Quaternion.AngleAxis
