...
/// <summary>
/// Physique de corps rigide – VERSION FINALE AVEC FRACTURE
/// </summary>
public class RempartPhysique : MonoBehaviour
{
    [Header("Propriétés Physiques")]
    public float masse = 5f;
    public Vector3 sizeUnity = new Vector3(1f, 1f, 1f);
    private MyVector3 size;

    [Header("Centre d'Inertie")]
    public Vector3 centreInertieLocal = Vector3.zero;

    [Header("Coefficients")]
    public float coefficientRestitution = 0.2f;
    public float friction = 0.93f;
    public float frictionAngulaire = 0.85f;

    [Header("Résistance de l'Air")]
    public bool utiliserResistanceAir = true;
    public float coefficientTrainee = 0.47f;
    public float densiteAir = 1.225f;

    [Header("Correction de Pénétration")]
    public float pourcentageCorrection = 0.99f;
    public float seuilPenetration = 0.00001f;
    public float separationMinimale = 0.01f;

    [Header("Limites de Sécurité")]
    public float vitesseMax = 30f;
    public float vitesseAngulaireMax = 15f;
    ...
    private MyVector3 CentreInertieGlobal
    {
        get
        {
            MyVector3 localOffset = MyVector3.FromUnity(centreInertieLocal);
            Matrix3x3 rotMatrix = rotation.ToMatrix3x3();
            Vector3 rotatedOffsetUnity = rotMatrix.MultiplyVector(localOffset.ToUnity());
            MyVector3 rotatedOffset = MyVector3.FromUnity(rotatedOffsetUnity);
            return MyVector3.Add(position, rotatedOffset);
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        persistentContacts.RemoveAll(c => c.frameCount > 10);
    ...
    void OnDrawGizmos()
    {
        if (Application.isPlaying)
        {
            ...
            Gizmos.DrawSphere(CentreInertieGlobal.ToUnity(), 0.1f);
            Gizmos.color = Color.yellow;
            Gizmos.DrawLine(transform.position, CentreInertieGlobal.ToUnity());
        }
    }

    private void LimiterVitesses()
    {
        float vMag = vitesse.Magnitude();
        if (vMag > vitesseMax)
        {
            vitesse = MyVector3.Scale(vitesse.Normalized(), vitesseMax);
            quantiteMouvement = MyVector3.Scale(vitesse, masse);
        }

        float wMag = vitesseAngulaire.Magnitude();
        if (wMag > vitesseAngulaireMax)
        {
            vitesseAngulaire = MyVector3.Scale(vitesseAngulaire.Normalized(), vitesseAngulaireMax);
        }
    }

    private void CalculerTensorInertieLocal()
    {
        float lx = size.x, ly = size.y, lz = size.z;
    ...
    {
        MyVector3 x0 = position; MyQuaternion R0 = rotation;
        MyVector3 P0 = quantiteMouvement; MyVector3 L0 = momentAngulaire;

        UpdateTensorInertieMondeReel();
        MyVector3 v1 = vitesse; MyQuaternion Rdot1 = CalculerDeriveeRotation(rotation, vitesseAngulaire);
        MyVector3 F1 = CalculerForcesExternes(); MyVector3 Tau1 = CalculerTorquesExternes();
        ...
        UpdateTensorInertieMondeReel();
        MyVector3 v2 = vitesse; MyQuaternion Rdot2 = CalculerDeriveeRotation(rotation, vitesseAngulaire);
        MyVector3 F2 = CalculerForcesExternes(); MyVector3 Tau2 = CalculerTorquesExternes();
        ...
        UpdateTensorInertieMondeReel();
        MyVector3 v3 = vitesse; MyQuaternion Rdot3 = CalculerDeriveeRotation(rotation, vitesseAngulaire);
        MyVector3 F3 = CalculerForcesExternes(); MyVector3 Tau3 = CalculerTorquesExternes();
        ...
        UpdateTensorInertieMondeReel();
        MyVector3 v4 = vitesse; MyQuaternion Rdot4 = CalculerDeriveeRotation(rotation, vitesseAngulaire);
        MyVector3 F4 = CalculerForcesExternes(); MyVector3 Tau4 = CalculerTorquesExternes();
    ...
    private MyVector3 CalculerForcesExternes()
    {
        ...
        if (utiliserResistanceAir)
        {
            ...
            if (vitesseCarree > 0.001f)
            {
                ...
                float magnitudeTrainee = 0.5f * densiteAir * vitesseCarree * coefficientTrainee * aireReference;
    ...
    private MyVector3 CalculerTorquesExternes()
    {
        ...
        if (utiliserResistanceAir)
        {
            ...
            if (omegaCarree > 0.001f)
            {
                ...
                MyVector3 torqueResistance = MyVector3.Scale(vitesseAngulaire, -facteurResistanceAngulaire * omegaCarree);
    ...
    private void CollisionAvecSol()
    {
        float niveauSol = 0f;
        float demiHauteur = size.y * 0.5f;
        float positionBas = position.y - demiHauteur;

        if (positionBas < niveauSol + separationMinimale)
        {
            float penetration = niveauSol + separationMinimale - positionBas;
            position.y += penetration;

            MyVector3 pointContactLocal = new MyVector3(0, -demiHauteur, 0);
            MyVector3 vitesseAuContact = MyVector3.Add(vitesse, MyVector3.Cross(vitesseAngulaire, pointContactLocal));
            float vitesseNormale = vitesseAuContact.y;

            if (vitesseNormale < 0)
            {
                float e = 0.1f;
                float numerateur = -(1f + e) * vitesseNormale;
                float masseInv = 1f / masse;

                MyVector3 normaleSol = new MyVector3(0, 1, 0);
                MyVector3 rCrossN = MyVector3.Cross(pointContactLocal, normaleSol);
                Vector3 invI_rCrossN = tensorInertieInverseMonide.MultiplyVector(rCrossN.ToUnity());
                MyVector3 termRotation = MyVector3.Cross(MyVector3.FromUnity(invI_rCrossN), pointContactLocal);
                float denominateur = masseInv + MyVector3.Dot(termRotation, normaleSol);

                if (Mathf.Abs(denominateur) > 0.0001f)
                {
                    float j = numerateur / denominateur;
                    MyVector3 impulsion = MyVector3.Scale(normaleSol, j);
                    quantiteMouvement = MyVector3.Add(quantiteMouvement, impulsion);
                    MyVector3 torque = MyVector3.Cross(pointContactLocal, impulsion);
                    momentAngulaire = MyVector3.Add(momentAngulaire, torque);
                    UpdateTensorInertieMondeReel();
                }
            }

            quantiteMouvement.x *= 0.90f;
            quantiteMouvement.z *= 0.90f;
            momentAngulaire = MyVector3.Scale(momentAngulaire, 0.80f);
        }
    }
    ...
    public void ResoudreCollision(RempartPhysique autre)
    {
        ...
        MyVector3 centreA = CentreInertieGlobal;
        MyVector3 centreB = autre.CentreInertieGlobal;
        MyVector3 AB = MyVector3.Subtract(centreB, centreA);

        if (MyVector3.Dot(normale, AB) < 0)
            normale = MyVector3.Scale(normale, -1f);

        float masseInvA = 1f / masse;
        float masseInvB = 1f / autre.masse;

        // ✅ AJOUTER: Track impulsion max
        float maxImpulsion = 0;

        foreach (MyVector3 pointContact in collisionInfo.contactPoints)
        {
            ContactPoint contact = TrouverOuCreerContact(pointContact, normale, collisionInfo.penetrationDepth);

            MyVector3 r1 = MyVector3.Subtract(pointContact, centreA);
            MyVector3 r2 = MyVector3.Subtract(pointContact, centreB);
            ...
            // ✅ AJOUTER: Track impulsion max
            maxImpulsion = Mathf.Max(maxImpulsion, Mathf.Abs(j));
        ...
        {
            float correction = (collisionInfo.penetrationDepth + separationMinimale) * pourcentageCorrection;
            float totalMasseInverse = masseInvA + masseInvB;

            if (totalMasseInverse > 0.0001f)
            {
                ...
                autre.position = MyVector3.Subtract(autre.position, MyVector3.Scale(deplacement, masseInvB));
            }
        }

        // ✅ FRACTURE ACTIVATION AVEC IMPULSION CORRECTE
        FractureSystem fractureSys = FindObjectOfType<FractureSystem>();
        if (fractureSys != null && maxImpulsion > 0)
        {
            fractureSys.TesterFracture(this, autre, collisionInfo.contactPoints[0], collisionInfo.normal, maxImpulsion);
        }
    }
    ...
    {
        MyVector3 r = MyVector3.Subtract(pointApplication, CentreInertieGlobal);
